...
final currentItemsProvider =
    NotifierProvider.family<CurrentItemsNotifier, List<PluginItem>, int>(
        () => CurrentItemsNotifier());
...
class CurrentItemsNotifier extends FamilyNotifier<List<PluginItem>, int> {
  ...
  List<PluginItem> build(int maxElement) {
    ...
    if (currentPage > 0 && items.length > 0) {
      final to = (currentPage + 1) * maxElement > items.length
          ? items.length
          : (currentPage + 1) * maxElement;
  ...
  void next(int maxElement) {
    ...
    if (allItems.length > maxElement * nextPage) {
      final to = nextPage * maxElement + maxElement > allItems.length
          ? allItems.length
          : nextPage * maxElement + maxElement;
  ...
  void prev(int maxElement) {
    ...
    if (currentPage > -1) {
      state = allItems.sublist(
          currentPage * maxElement, (currentPage + 1) * maxElement);
      ref.read(currentPageProvider.notifier).state -= 1;
    }
  }
}
...
class Wheel extends ConsumerWidget {
  ...
  _updateHoverSection(PointerEvent event, Size size, double sectionAngle,
      double centerSize, WidgetRef ref) {
    ...
    if (distToCenter > radius ||
        distToCenter < (centerSize / size.shortestSide)) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    switch (theme.menuFontColor) {
      ...
      case t.ThemeColorGradient():
        menu_paint.shader = (theme.menuFontColor as t.ThemeColorGradient)
            .value
      ...
      child: Listener(
        ...
        onPointerUp: (event) {
          ...
          ref
              ...
              .onClick(currentItems[ref.read(hoveredSectionProvider) - 1]);
        },
        onPointerSignal: (pointerSignal) {
          if (pointerSignal is PointerScrollEvent) {
            if (pointerSignal.scrollDelta.direction.isNegative)
              ref
                  .read(currentItemsProvider(maxElement).notifier)
                  .prev(maxElement);
            else
              ref
                  .read(currentItemsProvider(maxElement).notifier)
        ...
        child: Stack(
          children: [
            ...
            ...getMenuItems(currentItems, size, sectionAngle, theme, centerSize,
                themeBrightness),
            Center(
              child: Container(
...
class WheelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    switch (backgroundColor) {
      ...
      case t.ThemeColorGradient():
        bg_paint.shader = (backgroundColor as t.ThemeColorGradient)
            .value
            .createShader(Rect.fromCenter(
                center: center, width: shortSide, height: shortSide));
    }
    ...
    switch (separatorThickness) {
      ...
      case t.AONInt():
        sp_paint.strokeWidth =
            (separatorThickness as t.AONInt).value.toDouble();
    }
    switch (separatorColor) {
      case t.ThemeColorSolid():
        sp_paint.color = (separatorColor as t.ThemeColorSolid).value;
      case t.ThemeColorGradient():
        sp_paint.shader = (separatorColor as t.ThemeColorGradient)
            .value
            .createShader(Rect.fromCenter(
                center: center.translate(shortSide / 4, 0),
                width: shortSide / 2,
                height: shortSide / 2));
    }
    ...
    switch (centerColor) {
      ...
      case t.ThemeColorGradient():
        ct_paint.shader = (centerColor as t.ThemeColorGradient)
            .value
            .createShader(Rect.fromCenter(
                center: center, width: centerSize * 2, height: centerSize * 2));
    }
    ...
    canvas.drawCircle(
        center, (shortSide / 2) - (ol_paint.strokeWidth / 2), ol_paint);
  }
...
List<Positioned> getMenuItems(
  ...
  final squareLength =
      calculateMaxSquare(radius, min(sectionAngle * 0.5, pi / 5));
  ...
  for (int i = 1; i <= items.length; i++) {
    final angle =
        items.length == 1 ? pi * 0.5 : sectionAngle * i - sectionAngle * 0.5;
    ...
    switch (theme.itemFontColor) {
      ...
      case t.ThemeColorGradient():
        item_paint.shader = (theme.itemFontColor as t.ThemeColorGradient)
            .value
            .createShader(Rect.fromCenter(
                center: Offset(radius + cos(angle) * distance,
    ...
    switch (theme.descriptionFontColor) {
      case t.ThemeColorSolid():
        desc_paint.color =
            (theme.descriptionFontColor as t.ThemeColorSolid).value;
      case t.ThemeColorGradient():
        desc_paint.shader = (theme.descriptionFontColor as t.ThemeColorGradient)
            .value
            .createShader(Rect.fromCenter(
                center: Offset(radius + cos(angle) * distance,
                    radius - sin(angle) * distance),
